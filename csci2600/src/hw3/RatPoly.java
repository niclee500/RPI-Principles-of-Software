...
public final class RatPoly {
    ...
    public RatPoly(int c, int e) {
        // spec says if c is zero, create a "0" polynomial, which is like the default constructor
        if(c == 0){
            coeffs = new RatNum[0];
            degree = 0;
        } else{
            degree = e;
            coeffs = new RatNum[degree+1];
            for(int i = 0; i < degree; ++i){ coeffs[i] = new RatNum(0); }
    ...
    public RatNum getCoeff(int pow) {
        ...
        } else if( (degree == 0) || (pow < 0) || (pow >= coeffs.length) ){
            return new RatNum(0);
        } else{
            return coeffs[pow];
        }
    ...
    public RatPoly negate() {
        if(this.isNaN()){
            ...
            RatNum[] neg = new RatNum[degree+1];
            for(int i = 0; i <= degree; ++i){
                neg[i] = coeffs[i].negate();
            }
            scaleCoeff( coeffs, new RatNum(-1) );
            return new RatPoly(neg);
        }
    }
    ...
    public RatPoly add(RatPoly p) {
        if( this.isNaN() || p.isNaN() ){
            ...
            if(coeffs.length > p.coeffs.length){
                ...
                nsize = p.coeffs.length;
            }
            RatNum[] combo = new RatNum[nsize];
            for(int d = 0; d < nsize; ++d){
                if( d < coeffs.length && d < p.coeffs.length ){
                    combo[d] = coeffs[d].add(p.coeffs[d]);
                } else if(d < coeffs.length){
                    combo[d] = coeffs[d];
                } else{
                    combo[d] = p.coeffs[d];
                }
            ...
            if(resize == 0){
                return RatPoly.ZERO;
            } else if(resize < nsize){
                RatNum[] combo2 = new RatNum[resize];
                for(int i = 0; i < resize; ++i){
                    combo2[i] = combo[i];
                }
                return new RatPoly(combo2);
            }
            return new RatPoly(combo);
        }
    }
    ...
    public RatPoly mul(RatPoly p) {
        if(this.isNaN() || p.isNaN()){
            return RatPoly.NaN;
        } else if(degree == 0 || p.degree == 0){
            return RatPoly.ZERO;
        } else{
            int rdegree = degree + p.degree;
            RatNum[] result = new RatNum[rdegree+1];
            for(int r = 0; r <= rdegree; ++r){ 
                result[r] = RatNum.ZERO; 
            }
            for(int i = 0; i < coeffs.length; ++i){
                for(int j = 0; j < p.coeffs.length; ++j){
                    RatNum temp = coeffs[i].mul(p.coeffs[j]);
    ...
    * @requires p != null
    * @return a RatPoly, q, such that this = "q * p + r", where r is a RatPoly for the remainder, 
    *         the degree of r is strictly less than the degree of p, the degree of q is no 
    *         greater than the degree of this, and r and q have no negative exponents;
    *         if p is zero or this.isNaN() or p.isNaN(), returns some q such that q.isNaN()
    */
    public RatPoly div(RatPoly p){
        // TODO: Extra credit, fill in div method
        if( (p == RatPoly.ZERO) || (this.isNaN()) || (p.isNaN()) ){
            return RatPoly.NaN;
        } else{
            return RatPoly.ZERO;
        }
    ...
    public double eval(double d) {
    	...
    	if(this.isNaN()){
            ...
            int i = degree;
            double val = coeffs[degree].doubleValue();
            while(i > 0){
                val = val * d + coeffs[i-1].doubleValue();
                i--;
            }
            return val;
        }
    ...
    public RatPoly differentiate() {
        if(this.isNaN()){
            ...
            RatNum[] deriv = new RatNum[coeffs.length-1];
            for(int i = 1; i < coeffs.length; ++i){
                deriv[i-1] = coeffs[i].mul(new RatNum(i));
            }
            return new RatPoly(deriv);
        }
    }

    /**
     * Returns the antiderivative of this RatPoly.
     * @param integrationConstant The constant of integration to use when computating the antiderivative.
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        if(this.isNaN() || integrationConstant.isNaN()){
            ...
            RatNum[] anti = new RatNum[coeffs.length+1];
            anti[0] = integrationConstant;
            for(int i = 0; i < coeffs.length; ++i){
                anti[i+1] = coeffs[i].mul(new RatNum(1, i+1));
            }
            return new RatPoly(anti);
        }
    }
    ...
    public double integrate(double lowerBound, double upperBound) {
        RatPoly bigF = antiDifferentiate(RatNum.ZERO);
        return bigF.eval(upperBound) - bigF.eval(lowerBound);
    }
